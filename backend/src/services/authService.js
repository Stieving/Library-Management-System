...
    // Create a new user instance with isVerified set to false
    const user = new User({
      username,
      email,
      password,
...
      verificationTokenExpire: Date.now() + 60 * 60 * 1000, // 1 hour expiration
    });
    await user.save();
...
    const message = `Please click the link to verify your account: ${verificationURL}`;

    try {
      // Use the sendEmail utility to send the verification email
      await sendEmail({
        email: user.email,
...
      // Return a success object without the JWT token, as the user is not yet verified
      return {
        success: true,
...
      await user.save();
      logger.error(`Failed to send verification email to ${user.email}: ${err.message}`, err);
      throw new AppError(500, 'There was an error sending the verification email. Please try again later.');
    }
  } catch (error) {
...
      throw new AppError(403, 'Your email has not been verified. Please check your inbox for the verification link.');
    }
...
/**
 * @desc Handles the logic for a user verifying their email.
...
export async function verifyEmailService(token) {
  ...
  // ✅ Verify account and clear token fields
...
export const resendVerificationEmailService = async (email) => {
  ...
  user.verificationTokenExpire = Date.now() + 60 * 60 * 1000; // 1 hour
  await user.save();
